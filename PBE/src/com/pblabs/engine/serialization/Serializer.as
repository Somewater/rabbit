...
package com.pblabs.engine.serialization
{
    ...
    public class Serializer
    {
		...
		public var componentMap:Dictionary;
		...
		/**
		 * FIX 11.03.2011. Обеспечить правильную генерацию 
		 * свободной позиции для персонажей, имеющих ненулевой размер
         * Возвратить текущую сериализуемую сущность
         */
        ...
        private function getChildObject(object:*, fieldName:String, typeName:String, fieldXml:XML):*
        {
			...
			// FIX: (Павел, 3 янв.: избежать ошибки, когда type означает имя callback в не класс)
			// before:
			// var desiredType:Class; = TypeUtility.getClassFromName(typeName);
			// after:
			var desiredType:Class;
			try{
				desiredType = TypeUtility.getClassFromName(typeName);
			}catch(e:Error){
				Logger.warn(this, "getChildObject", "flash.utils::getDefinitionByName error for \"" + typeName + "\". FIX 03.01.2011");
				desiredType = ActionScriptVersion;// присвоить дурацкий неупотребляемый тип, чтобы нижележащий if выполнился в любом случае
			}
			// end FIX
