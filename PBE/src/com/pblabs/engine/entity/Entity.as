package com.pblabs.engine.entity
{
    ...
    internal class Entity extends PBObject implements IEntity
    {
        ...
        public override function destroy():void
        {
			...
			signalListeners.splice(0, signalListeners.length);
			signalListeners = [];
        }
        ...
        public function deserialize(xml:XML, registerComponents:Boolean = true):void
        {
            ...
            for each (var componentXML:XML in xml.*)
            {
                ...
                if (componentClassName.length > 0 && _components[componentName] == null)
                {
					if(Serializer.instance.componentMap[componentName])
						componentClassName = Serializer.instance.componentMap[componentName];
        ...
        private function doAddComponent(component:IEntityComponent, componentName:String):Boolean
        {
            ...
            if (_components[componentName])
            {
                ...
                // FIX: 9.03.2011. Обеспечить замещение компонентов при наследовании
				/*
				return false;
				*/
				removeComponent(_components[componentName]);
            }
        ...
        private function doRemoveComponent(component:IEntityComponent):Boolean
        {
            ...
            if (!_components[component.name])
            {
				...
				// FIX: 9.03.2011. Обеспечить замещение компонентов при наследовании
				// правильно удалить компонент, который еще не был инициирован, однако был связан с owner
		...
		public function bind(signalName:String, callback:Function):void
		{
			var listeners:Array = signalListeners[signalName];
			if(!listeners)
			{
				listeners = [];
				signalListeners[signalName] = listeners;
			}
			
			listeners.push(callback);
		}
		public function unbind(signalName:String, callback:Function):void
		{
			var listeners:Array = signalListeners[signalName];
			if(listeners)
			{
		...
		public function signal(signalName:String):void
		{
			var listeners:Array = signalListeners[signalName];
			if(listeners)
			{
