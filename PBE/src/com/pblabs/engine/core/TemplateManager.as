	...
	public class TemplateManager extends EventDispatcher
	{
		...
		public function instantiateEntityFromXML(xml:XML):IEntity
		{
			...
			try
			{
				...
				Serializer.instance.componentMap = resetTypes(xml, new Dictionary());
		...
		public function instantiateGroup(name:String):Array
		{
			...
			{
				Logger.error(this, "instantiateGroup", "Failed to instantiate group '" + name + "' due to: " + e.getStackTrace());
				return null;
			}
		...
		public function getXML(name:String, xmlType1:String=null, xmlType2:String=null):XML
		{
			...
			return thing ? thing.xmlData.copy() : null;
		}
		...
		private function doInstantiateTemplate(object:IEntity, templateName:String, tree:Dictionary, componentMap:Dictionary):Boolean
		{
			...
			// вписать наиболее последние типы компонентов
			resetTypes(templateXML, componentMap);
			
			if (!doInstantiateTemplate(object, templateXML.attribute("template"), tree, componentMap))
				return false;
		...
		private function resetTypes(templateXML:XML, componentMap:Dictionary):Dictionary
		{
			for each (var componentXML:XML in templateXML.*)
			{
				if(componentXML.localName().toLowerCase() == "component")
				{
					var name:String = componentXML.attribute("name");
					var type:String = componentXML.attribute("type");
					
					if(type && type.length)
					{
						if(componentMap[name] == null)
						//	componentXML.@type = componentMap[name]
						//else
							componentMap[name] = type;
					}
				}
			}
		...
		/**
		 * FIX 11.03.2011 Обеспечить ввод координат в xml файле, отвечающем за описание левела
		 * FIX 24.09.2011 Обеспечить создание индивидуального хэша для каждой entity
		 */
		private var entitySpatialTile:PropertyReference = new PropertyReference("@Spatial.tile");
		private function setEntityParams(entity:IEntity, objectXML:XML):void
		{
			var x:String = objectXML.attribute("x");
			var y:String = objectXML.attribute("y");
			var hash:String = objectXML.attribute("hash");
			if(x || y)
			{
				entity.setProperty(entitySpatialTile, new Point(int(x), int(y)));
			}
		...
		public function getIndividualHash():String
		{
			...
			{
				var n:int = Math.random() * 36 + 48;
				if(n > 57) n += 39;
				hash += String.fromCharCode(n);
			}
			return hash;
		}
