...
package com.pblabs.engine.core
{
    ...
    public class ObjectType implements ISerializable
    {
         ...
         * use the various test methods on the ObjectTypeMana`er.
         * 
        ...
        public function ObjectType(...arguments)
        {
            if(arguments.length == 1)
            {
				...
				// FIX 25.01.2011 by Pavel Naydenov. Поддержка ввода нескольких типов как один String через запятую. Например "wild,evil,horse"
                else if(String(arguments[0]).indexOf(",") != -1)
					typeNames = String(arguments[0]).split(",");
				else
