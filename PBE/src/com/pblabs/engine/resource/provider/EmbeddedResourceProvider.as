package com.pblabs.engine.resource.provider
{
	...
	public class EmbeddedResourceProvider extends ResourceProviderBase
	{
        ...
        */
		// FIX 6.01.2011 by Naydenov Pavel. Обеспечить эмбед ресурсов, 
		// которые, если не заэмбежены, хранятся на удаленном сервере (не на том, где флешка)
		public function registerResource(filename:String, resourceType:Class, data:*, alias:String = null):void
        {
            ...
            try
            {
				...
				// которые, если не заэмбежены, хранятся на удаленном сервере (не на том, где флешка)
				if(alias)
					resources[alias + resourceType] = resource;
            }
            catch(e:Error)
            {
