...
[Event(name="autoCompleteChanged", type="com.yahoo.astra.fl.controls.AutoComplete")]


public class AutoComplete extends TextInput implements IClear
{
	public static const AUTO_COMPLETE_CHANGED:String = "autoCompleteChanged";
	public var EMPTY_LABEL:String = "< не указано >";
	...
	{
		background.removeEventListener(MouseEvent.CLICK, openOnClick);
		textField.removeEventListener(MouseEvent.MOUSE_DOWN, openOnClick);
		
		textField.removeEventListener(TextEvent.TEXT_INPUT, handleTextInput);
		textField.removeEventListener(Event.CHANGE, handleChange);
		textField.removeEventListener(KeyboardEvent.KEY_DOWN, handleKeyDown);
	}
	
	private var lastLabelFired:String = null;// итем с каким лабелом привел к диспетчеризации события в прошлый раз
	
	
	
	
	
	private function checkTypedText():void
	{
		var text:String = textField.text.toLowerCase();
		var data:Array = _dropdown.dataProvider.toArray();
		for(var i:int = 0;i<data.length;i++)
		{
			if(data[i][_labelField].toLowerCase() == text)
			{
				var tempB:Boolean = bInKeyDown;
				bInKeyDown = true;
				selectedIndex = i; 
				bInKeyDown = tempB;
				break;
...
//		
//		textField = new TruncatedTextField();
//		addChild(textField);
//		updateTextFieldType();
...
//		textField.addEventListener(MouseEvent.MOUSE_DOWN, openOnClick);
//	}
	
	private function openOnClick(e:Event):void
	{
		open();
	}
	...
	override protected function drawBackground():void
	{
		...
		background = new BackgroundSkin(true, !_enabled);
		background.alpha = _enabled?1:0.6;
		addChildAt(background, 0);
		background.addEventListener(MouseEvent.CLICK, openOnClick);
	}
	...
	override protected function drawLayout():void
	{
		...
		textField.width = width-txtPad - 40;
		textField.height = height-12;
		textField.x = txtPad;
    ...
    {
		value.addItemAt({name:EMPTY_LABEL, country_id:0, region_id:0, city_id:0}, 0);
        _dataProvider = value;
    ...
    public function set selectedIndex(value:int):void
    {
        _dropdown.selectedIndex = value;
         if ( value >=0)
         {
			...
			var label:String = selectedItem?selectedItem[_labelField]:null;
			if(label != lastLabelFired)
			{
				dispatchEvent(new Event(AutoComplete.AUTO_COMPLETE_CHANGED));
				lastLabelFired = label;
			}
        }
	 ...
	 */
	private var _autoFillEnabled:Boolean = true;
	/**
	 *  @private
	 */
	...
	protected function defaultFilterFunction(element:*, index:int = 0, arr:Array = null):Boolean 
	{
		...
		if(label == EMPTY_LABEL) return true;
...
import com.progrestar.assets.ComboBox_line_overSkin;
import com.progrestar.assets.ComboBox_line_upSkin;
import com.progrestar.assets.ComboBox_overSkin;
import com.progrestar.assets.ComboBox_thumb_overSkin;
import com.progrestar.assets.ComboBox_thumb_upSkin;
import com.progrestar.assets.ComboBox_upSkin;
...
class BackgroundSkin extends Sprite
{
	...
	public function BackgroundSkin(up:Boolean, stopped:Boolean)
	{
	...
	public function setSkins(up:Boolean):void
	{
		if(ground)	removeChild(ground);
		if(thumb) 	removeChild(thumb);
		if(line) 	removeChild(line);
		
		ground = addChild(up?new com.progrestar.assets.ComboBox_upSkin():new com.progrestar.assets.ComboBox_overSkin);
		thumb = addChild(up?new ComboBox_thumb_upSkin():new ComboBox_thumb_overSkin());
		line = addChild(up?new ComboBox_line_upSkin():new ComboBox_line_overSkin());
		
		setSize(_width, _height, true);
	}
	...
	public function setSize(w:Number, h:Number, f:Boolean = false):void
	{
		if((!f) && 
			_width == w && 
			_height == h) return;
		...
		thumb.scaleX = thumb.scaleY = h/40;
		thumb.x = w - thumb.width - 8;
		thumb.y = (h - thumb.height) * 0.5;
