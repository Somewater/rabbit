package com.somewater.social
{
		...
		
	{
		...
		override public function createSocialUser(info:Object):SocialUser{
			...
			socialUser.hasMobile = info['has_mobile'];
		...
		override public function get initObject():Object{
			...
			data['referrer']['type'] = flashVars['referrer'];
			
			if( flashVars['referrer'] == 'wall_view' ){
				data['referrer']['id'] = flashVars['poster_id'];	
			} /*else {
			data['referrer']['id'] = flashVars['user_id'];
			} */
			if((flashVars['user_id'].toString() != flashVars['viewer_id'].toString()) && flashVars["user_id"].toString() != '0') {
				data['referrer']['id'] = flashVars['user_id']; 
				data['referrer']['type'] = 'invitation';
			}
		...
		override public function wallPost(recipient:SocialUser=null, title:String=null, message:String=null, 
			...
			if(super.wallPost(recipient)){
				...
				postData = HashModem.modulate(HashModem.softReplacement(postData));// вырезать неподдерживаемые символы и закодировать
		...
		private function onWallPhotoSettingChange(e:Event) {
		...
		override protected function _createSignature(urlVariables:Object):void{
			...
			urlVariables["sig"] = MD5.encrypt(flashVars["viewer_id"] + signature + flashVars['secret']);
		...
		override protected function _sendRequest(method:String, request:Object=null, onComplete:Function=null, onError:Function=null, GET:Boolean=false):void
		{
			...
			super._sendRequest(method, request, onComplete, onError, GET);
		}
		...
		override protected function _getLogUrlVariables():URLVariables
		{
			...
			vars["app_id"] = app_id;
			vars["user_id"] = flashVars["viewer_id"];
			vars["url"] = _getApiUrl(null);
			return vars;
		}
	}
}