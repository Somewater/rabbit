package com.somewater.social
{
	...
	public class FacebookSocialAdapter extends SocialAdapter
	{
		...
		override public function get initObject():Object{
			...
			if(flashVars["referrer_id"]){
				obj["referrer"]["id"] = flashVars["referrer_id"];
				obj["referrer"]["type"] = "invintation";
			}else if(flashVars["oid"] && flashVars["oid"] != user.id){
				obj["referrer"]["id"] = flashVars["oid"];
				obj["referrer"]["type"] = "invintation";
			}
			return obj;
		}
		...
		override public function showInviteBox(uid:String = null, type:String = null, onComplete:Function = null, onError:Function = null):Boolean{
			if(ExternalInterface.available){
				var vars:String = "";
				if(type == "addteam.php"){
					vars = "pstar_loc=join," + uid + "&oid=" + user.id + "&vid=" + uid;
				}else{
					vars = "exclude_ids=" + appFriendsIds.join() + "&uid=" + user.id;
				}
				vars += (uid?"&uid=" + uid:"");
				...
				ExternalInterface.call("inviteFriends", (type?type:"invite.php"), vars);
				return true;
			}else
				return false;
		}
